...
class trainer(object):
    def __init__(self, model,train_loader=None,val_loader=None,max_epoch=1000,enable_training=True):
        # callbacks types
            ...
            while self.epoch < self.max_epoch:
                ...
                if self.enable_training:
                    ...
                    for i, data in enumerate(self.train_loader):
                        ...
                        time_elaps = time.time() - st 
                        examples_per_second = self.train_loader.batch_size / time_elaps
                        st = time.time()
                        bar.suffix=f"train: [{self.epoch}][{i}/{len(self.train_loader)}] | Examples/Second: {examples_per_second:.3f} | Total: {bar.elapsed_td:} | ETA: {bar.eta_td:} {summary['suffix']}"
                        bar.next()
                        #count_ += 1
                        #if count_ == 20:
