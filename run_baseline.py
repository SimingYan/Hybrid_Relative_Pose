...
def _parse_args():
    ...
    parser.add_argument('--method', type = str, default = 'gr', help = 'option 4pcs, gr')
...
if __name__ == '__main__':
    ...
    for i in range(num_data):
        ...
        elif args.dataset == 'suncg' or args.dataset == 'matterport':
    ...
    R_baseline = np.stack(R_baseline)
    ...
    if args.save_rp == 1:
        ...
        if args.dataset == 'scannet':
            sio.savemat('./data/test_data/scannet_source_%s.mat' % appending,{'path': path_s_l, 'pred_pose': pred_pose_l, 'gt_pose': gt_pose_l, 'overlap':overlap_vals})
            sio.savemat('./data/test_data/scannet_target_%s.mat' % appending,{'path': path_t_l, 'pred_pose': pred_pose_l, 'gt_pose': gt_pose_l, 'overlap':overlap_vals})

        elif args.dataset == 'suncg':
            sio.savemat('./data/test_data/suncg_source_%s.mat' % appending,{'path': path_s_l, 'pred_pose': pred_pose_l, 'gt_pose': gt_pose_l, 'overlap':overlap_vals})
            sio.savemat('./data/test_data/suncg_target_%s.mat' % appending,{'path': path_t_l, 'pred_pose': pred_pose_l, 'gt_pose': gt_pose_l, 'overlap':overlap_vals})
        elif args.dataset == 'matterport':
            sio.savemat('./data/test_data/matterport_source_%s.mat' % appending,{'path': path_s_l, 'pred_pose': pred_pose_l, 'gt_pose': gt_pose_l, 'overlap':overlap_vals})
            sio.savemat('./data/test_data/matterport_target_%s.mat' % appending,{'path': path_t_l, 'pred_pose': pred_pose_l, 'gt_pose': gt_pose_l, 'overlap':overlap_vals})
