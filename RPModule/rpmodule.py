...
def fit_new_spectral(allSP,allTP,allSN,allTN,allWP,allWN,w_i1i2j1j2,mu,row,col,num_s,num_t,sourcePC,targetPC):
    """
    allSP: source keypoint position
    allTP: target keypoint position
    allSN: source keypoint normal
    ...
    # construct the A matrix, compute most significant eigenvector
    A = csc_matrix((a_i1i2j1j2, (row,col)), shape=(num_s*num_t, num_s*num_t))
    A = A+A.T
    ...
    num_rp = 3
    pair_cluster = KMeans(n_clusters=num_rp, random_state=0).fit(u_)
        
    R_hat_l = []


    for k in range(num_rp):
        pair_idx = np.arange(0, num_s*num_t)
        #import pdb;pdb.set_trace()
        source_idx = pair_idx[pair_cluster.labels_==k] // num_t
        target_idx = pair_idx[pair_cluster.labels_==k] % num_t

        filter_s = sourcePC[source_idx]
        filter_t = targetPC[target_idx]
        filter_s_mean = filter_s.sum(0) / np.shape(filter_s)[0]
        filter_sc = filter_s - filter_s_mean

        filter_t_mean = filter_t.sum(0) / np.shape(filter_t)[0]
        filter_tc = filter_t - filter_t_mean

        R_tmp = horn87_np(filter_sc.T, filter_tc.T)
        t_tmp = -np.matmul(R_tmp.reshape(3,3), filter_s_mean.squeeze()) + filter_t_mean
        R_44_tmp = np.eye(4)
        R_44_tmp[:3,:3] = R_tmp
        R_44_tmp[:3,3] = t_tmp
...
def RelativePoseEstimation_helper(dataS, dataT,para):
    ...
    pcWij = np.expand_dims(sourcePCw,1)*np.expand_dims(targetPCw,0) # whether a pair of keypoints are all in the observed region
    dij = np.power(np.expand_dims(sourceDess,1)-np.expand_dims(targetDess,0),2).sum(2) # distance between features of keypoints
    ...
    elif para.method == 'multi_spectral':
        row = corres[0,idy]*numFea_t+corres[1,idy]
        col = corres[0,idx]*numFea_t+corres[1,idx]
        return fit_new_spectral(allSP,allTP,allSN,allTN,allWP,allWN,w_i1i2j1j2,para.mu,row,col,numFea_s,numFea_t, sourcePC, targetPC)
