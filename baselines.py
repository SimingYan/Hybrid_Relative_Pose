...
if util.eldar():
  ...
  compute_fpfh_feature = o3d.registration.compute_fpfh_feature
  KDTreeSearchParamHybrid = o3d.geometry.KDTreeSearchParamHybrid
  ...
  registration_fast_based_on_feature_matching = o3d.registration.registration_fast_based_on_feature_matching
  FastGlobalRegistrationOption = o3d.registration.FastGlobalRegistrationOption
else:
  from open3d import *
...
def super4pcs(pc_src, pc_tgt):
    ...
    if 1:
        ...
        #util.write_ply(filenameS, pc_src)
        #util.write_ply(filenameT, pc_tgt)
        util.pc2obj(filenameS,pc_src.T)
        util.pc2obj(filenameT,pc_tgt.T)

        filenameO = f"tmp/{str(uuid.uuid4())}"
        cmd = './Super4PCS -i {obj0} {obj1} -o 0.5 -r 4pcs_fast.obj -d 0.01 -t 1000 -n 200 -m {fileO}'.format(obj0=filenameT,obj1=filenameS,fileO=filenameO)
    try:
...
def preprocess_point_cloud(pcd, voxel_size):
    ...
    if util.eldar():
        pcd_down.estimate_normals(KDTreeSearchParamHybrid(
...
def open3d_color_registration(pc_src,pc_tgt,color_src,color_tgt):
    ...
    for scale in range(3):
        ...
        if util.eldar():
            source_down = pcd_src.voxel_down_sample(radius)
            target_down = pcd_tgt.voxel_down_sample(radius)
        else:
